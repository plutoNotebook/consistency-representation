...
class NCSNppEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.out = nn.Sequential(
            ...
            nn.SiLU(),
            nn.AdaptiveAvgPool2d((1, 1)),  # Pooling to (1, 1) size
            nn.Conv2d(in_ch, in_ch, 1),  # Conv layer to reduce channels to out_channels
    ...
    def forward(self, x):
        ...
        final_output = self.out(hs[-1])  # Apply the new output block to the last feature map

        return final_output  # Return intermediate outputs and the final encoded output